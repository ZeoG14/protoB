...
public final class EditorPane extends AbstractPane
{
	...
	public EditorPane(Controller controller)
	{
		...
		//	movies.add(new Movie(item, director));
	...
	private void	updateMoviePropertiesInModel()
	{
		...
		controller.set("movie.numberOfReviews", movie.getReviews());
		controller.set("movie.genre", movie.getGenre());

		controller.set("movie.award.picture", movie.isPicture());
		controller.set("movie.award.directing", movie.isDirecting());
		controller.set("movie.award.cinematography", movie.isCinematography());
		controller.set("movie.award.acting", movie.isActing());
